...
# Open the webcam using a Windows-friendly backend and verify access.
...
if cap is None:
    raise RuntimeError(
        "Cannot open camera 0. Close other camera apps, try a different camera index, or run with a supported webcam backend."
    )
